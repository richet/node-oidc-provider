...
module.exports = (provider) => {
    ...
    async (ctx) => {
      ...
      if (hint !== undefined) {
        ...
        const decoded = await IdToken.validate(hint, client).catch((err) => {
          throw new InvalidRequestError(`could not validate id_token_hint (${err.message})`);
        });
  ...
  return async function interactions(ctx, next) {
    ...
    if (interaction) {
      ...
      const interactionSession = new provider.Session(ctx.oidc.uuid, base64url.encode({
        returnTo,
        interaction,
        uuid: ctx.oidc.uuid,
...
console.log("****")
console.log(interactionSession);
console.log("*******")
console.log(provider.cookieName('interaction'))
console.log(ctx.oidc.uuid)
